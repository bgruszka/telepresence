...
## Version 2.25.0
## <div style="display:flex;"><img src="images/feature.png" alt="feature" style="width:30px;height:fit-content;"/><div style="display:flex;margin-left:7px;">Personal Intercepts: Header-based Traffic Filtering</div></div>
<div style="margin-left: 15px">

Telepresence now supports Personal Intercepts, enabling developers to selectively intercept HTTP traffic based on custom headers and path patterns. This feature allows multiple developers to work in shared environments without interfering with each other's traffic.

**Key Features:**
- **Header Filtering**: Use `--header "X-User-ID=dev123"` to intercept only requests with specific headers
- **Path Filtering**: Use `--path "/api/v1/*"` to intercept only requests matching URL patterns
- **Combined Filters**: Combine multiple headers and paths for precise traffic targeting
- **Wildcard Support**: Use patterns like `--header "X-User-ID=dev*"` for flexible matching
- **Backward Compatible**: Existing TCP intercepts continue working unchanged

**Usage Examples:**
```bash
# Intercept traffic with specific header
telepresence intercept web-app --http --header "X-User-ID=dev123" --port 8080

# Multiple header filters (AND logic)
telepresence intercept api --http --header "X-Environment=staging" --header "X-Feature=checkout" --port 3000

# Combined header and path filtering
telepresence intercept api --http --header "X-User-ID=dev*" --path "/api/v2/*" --port 3000
```

This addresses the community request in [GitHub Issue #3852](https://github.com/telepresenceio/telepresence/issues/3852) for header-based traffic filtering in the open-source version.
</div>

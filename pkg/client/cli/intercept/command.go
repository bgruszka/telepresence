...
func (c *Command) AddInterceptFlags(cmd *cobra.Command) {
	...
	// Personal Intercepts flags
	flagSet.StringSliceVar(&c.HeaderFilters, "header", nil,
		`HTTP header filters for Personal Intercepts. Only requests with matching headers will be intercepted. `+
	...
	flagSet.StringSliceVar(&c.PathFilters, "path", nil,
		`HTTP path filter patterns for Personal Intercepts. Only requests matching these paths will be intercepted. `+
			`Supports glob patterns like "/api/v1/*". Requires --http flag.`)

	flagSet.BoolVar(&c.HTTPMechanism, "http", false,
		`Enable HTTP-aware interception for Personal Intercepts. When enabled, allows filtering by headers and paths. `+
...
func (c *Command) Validate(cmd *cobra.Command, positional []string) error {
	...
	// Personal Intercepts validation
